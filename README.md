...
2. `python tools/pod.py check [--rule R] [--files …]` → informe de defectos. Reglas:
   ...
   - `concord`: artículos o adjetivos que no concuerdan con lo que muestra una variable (`$pod_clan$`, `[doctrine|E]`, `[Glossary('X',…)]`, `[GetTrait('x').GetName(…)]`, `[UmbraGlossary('x')]`). El género y el número de esa palabra se deducen de su uso en el texto ya traducido (y de la terminación: -ción, -dad…); los epicenos e invariables (Kuei-Jin, clanes…) se declaran en `concord_overrides` de `tools/config.json` (`"--"` = no comprobar; también `"ms"`, `"fp"`, `"-s"`…). Los lotes de traducción y revisión traen en la cabecera cómo se ve cada variable para que el agente acierte a la primera.
